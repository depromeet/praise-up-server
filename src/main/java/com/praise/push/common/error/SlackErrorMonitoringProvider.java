...
public class SlackErrorMonitoringProvider implements ErrorMonitoringProvider {
    ...
    public void push(final Exception exception, final HttpServletRequest request) {
        ...
        slackApi.call(getSlackMessage(exception, request));
    }

    /**
     * @return Slack error message with exception and request
     */
    private SlackMessage getSlackMessage(final Exception exception, final HttpServletRequest request) {
        return new SlackMessage()
                .setUsername("Praise-Push Error Monitoring Bot")
                .setText("5xx Error is Detected")
                .setAttachments(getSlackAttachments(exception, request))
     ...
     */
    private List<SlackAttachment> getSlackAttachments(final Exception exception, final HttpServletRequest request) {
        SlackAttachment slackAttachment = new SlackAttachment()
                .setFallback("Error")
                .setColor("danger")
                .setTitleLink(request.getContextPath())
                .setText(Arrays.toString(exception.getStackTrace()))
                .setFields(getSlackFields(request));
     ...
     */
    private List<SlackField> getSlackFields(final HttpServletRequest request) {
        return List.of(
